...
Audited repository baseline: `f1361e26` (`main` and `origin/main`, including merged PRs #354–#356)
...
3. **Commission data inside transactions** already supports percentage or fixed commission, VAT treatment, agency and agent shares, referral split, other deductions, expected payment date, paid date, and status. The thin part is the standalone commission operating experience and reconciliation workflow.
...
| 5 | Create complete listings and publish correctly | High | **Strong foundation** | B/U/C/I | Inventory readiness, approval states, private revisions, assignment risks, missing fields/media, stale work, public metrics, and publication mismatch concepts exist. Listing readiness now receives a guarded mandate handoff. Browser acceptance for the complete create-review-publish path and production proof are not present in the agency acceptance suite. Syndication, duplicate-property checks, bulk actions, media QA, renewals, and seller reporting remain gaps. | First-pass approval rate; time to publish; publication mismatch rate | P0 validation, then P1 |
