...
| `PAID_MVP_SALES_PAUSED` | `false` unless testing an approved pause | `false` during staffed sales; `true` before a planned founder absence | non-secret boolean; exact `true` or `false` | B14 explicit override pauses new invoices and finance activation while preserving existing paid access; change requires reviewed runtime configuration and restart |
| `PAID_MVP_SALES_OPEN_UNTIL` | short UTC deadline for an approved expiry rehearsal | founder-renewed UTC deadline no later than the next weekday check and never more than 72 hours ahead; absent while explicitly paused | non-secret UTC timestamp | B14 automatically pauses new invoices and finance approval at expiry even if the API remains running; hosted paid release without a deadline stays paused |
