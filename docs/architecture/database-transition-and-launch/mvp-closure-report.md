...
path can make inventory public. The dedicated Land path and generic Developer
Land escape route are now hard-contained on the task branch; their integration,
protected-release verification, and separate
commercial/acceptance policy remain open. These findings and their launch disposition are recorded
...
| Direct administrator review-route authority containment (task branch) | `ddbb7e5893d2bd65361103f227887fd4c1f2e9b1` |
...
Goals 7–8, public enquiry custody and assigned-agent CRM continuity, are locally
verified at [`c4df6600`](https://github.com/Doscoding187/real_estate_portal/commit/c4df6600).
The existing real HTTP/tRPC acceptance now submits an anonymous public enquiry
for the newly published agency inventory, records its primary `crm_export`
custody row, verifies exact replay returns the same lead and a changed payload
is rejected, and checks that an unrelated verified agency has neither visibility
nor retry authority. The assigned agent performs a pipeline transition, contact
activity, follow-up, and ISO-timestamped viewing booking. When the agency term
expires, new opportunity capability becomes false but legitimate existing custody
remains available. Suspension through the canonical membership route denies the
former agent, moves the active lead and future showing to an invited replacement,
preserves the original custody delivery history, and allows the replacement and
agency owner to continue work.

The implementation separates `canReceiveLeads` from
`canAccessExistingLeads`, scopes CRM routes to current canonical membership, and
normalizes browser ISO showing times for the strict MySQL timestamp schema. The
governed integration passed **2 files / 2 tests** on the retained target; related
client/server regressions passed **7 files / 38 tests**, with one
...
### Canonical private-work membership authority supplement (2026-09-15)

A follow-up source audit found three private agency surfaces still treating a
retained profile projection as tenant authority after the Goal 1 correction:
seller-prospect access and assignment in the canvassing workflow, and legacy
agency analytics. A membership lifecycle deliberately retains `users.agencyId`
and `agents.agencyId` for history after suspension or removal. The affected
boundaries consulted those retained values and approved agent status without
resolving current canonical membership. That could let a lapsed member read or
work private seller-prospect data, remain selectable for a new assignment, or
read agency analytics. This is separate from the already-verified public lead
custody path; no lead-custody code was reopened.

Commit [`5f37d3d1`](https://github.com/Doscoding187/real_estate_portal/commit/5f37d3d1c14130f9078a17ea267e951547e65564)
derives non-manager seller-prospect scope and assignability from
`resolveCurrentAgencyMembershipForAgent` and the central current-membership
batch resolver. The resulting agency ID comes from the membership row, not the
user or agent profile. `canvassing.listAssignableAgents` now begins from the
canonical membership relationship and rejects expired, suspended, left, and
ambiguous affiliations. The legacy agency-analytics guard applies the same
exact-tenant membership decision. Agency-principal and super-admin paths retain
their existing authority rules.

On the exact owned disposable target, an active member reached the private
workspace and remained assignable; after suspension, the test retained both
legacy agency IDs and proved seller-prospect workspace/list denial, assignment
denial, assignment-dropdown exclusion, and agency-analytics denial. The
authority-wrapped focused run passed **2 files / 17 tests**; a related
membership, entitlement, listing, and lead-boundary run passed **7 files / 56
tests**. `pnpm check`, `pnpm test:db-authority:static`, targeted ESLint with no
errors, and `git diff --check` passed. The older canvassing integration is
deliberately gated to a separate `listify_test` target and therefore skipped on
this task-owned target; its fixture now creates canonical memberships for its
own governed invocation. This is not a claim that that separate integration
ran here.

No schema, migration, target lifecycle, payment, entitlement, publication,
provider, deployment, or protected operation changed. This closes the observed
...
### Canonical Agent Home workspace membership supplement (2026-09-15)

The next exact-target audit found a matching stale-affiliation gap in Agent
Home. The lead and calendar paths already resolved current canonical membership,
but Agent Home inventory, dashboard statistics, listing scheduling options,
commission views/CSV export, and generic property mutation started from the
retained agent profile. The direct pre-fix counterexample suspended canonical
membership while retaining the historical agency IDs, then received the full
private property row from `agent.getMyListings`.

Commit [`a0fc9e8b`](https://github.com/Doscoding187/real_estate_portal/commit/a0fc9e8b95be1275844e11aaed4bf57bc1ff2bd3)
adds a shared Agent Home workspace resolver. It permits unaffiliated agents and
no-profile preparation paths as before, but an agent record carrying an agency
affiliation must resolve a current canonical membership before the listed
operational paths proceed. Lead custody continues to require an approved
profile; property mutation uses the same affiliation decision before ownership
checks. This is not a commercial, publication, or Land-policy change.

The persisted-state regression first failed with the suspended member's full
private inventory record. After correction it retains the stale profile
projections and proves `FORBIDDEN` for Agent Home inventory, dashboard,
scheduling options, commissions, commission export, property update, and
property archive. The authority-wrapped final run passed **3 files / 18 tests**
(`integration.agency-membership-authority`, `agent.dashboard-showings.smoke`,
and `integration.agent-lead-transitions`) on target fingerprint
`a560e9f2971e7676…`. The non-database canonical showings and inventory contract
run passed **2 files / 9 tests**. `pnpm check`, `pnpm test:db-authority:static`
(**35 files / 295 tests**), targeted ESLint with zero errors, and `git diff
--check` passed.

This is local task-branch authority evidence only. It does not claim hosted
integration, provider operation, production verification, payment or
...
integration tests were skipped because that invocation had no `DATABASE_URL`.

Goals 7–8 then exercised the missing connected agency slice and found a material
CRM continuity defect: the screen-wide `canReceiveLeads` commercial gate could
strand an otherwise authorized assignee after the agency term elapsed. The
`c4df6600` correction separates new-opportunity receipt from existing-custody
access, enforces current canonical membership at the agent/agency lead routes,
and reassociates future active showings with lead reassignment. Its exact-target
acceptance proves the correction rather than merely asserting it. Goal 9 then
proved the supported first-cohort recovery fallback: a deliberately interrupted
platform-managed delivery is visible in the real super-admin audit/queue and can
be completed once with an auditable, replay-safe action. The procedure is
recorded in the [first-cohort recovery runbook](../launch-readiness-and-product-convergence/16-first-cohort-lead-custody-recovery-runbook.md).
Actual provider delivery, live worker supervision, and protected recovery remain
separate launch evidence requirements.
...
## LRC-LAND-001 containment follow-up

Senior review approved the hard-deferred Land disposition and two bounded
corrections. Commit [`335838df`](https://github.com/Doscoding187/real_estate_portal/commit/335838dff0c746b860eaaf2930412d4b38540db5)
keeps `deferred_first_cohort` immutable and disabled, without an environment
switch, payment bypass, migration, or changes to Land geography or public
classifications. Direct Land public, author, evidence, reviewer, and lifecycle
paths fail closed before a database mutation; generic listing, media, review,
public-projection, analytics, saved-search, sitemap, and navigation paths
exclude Land. The separate generic Developer `developments` model was outside
that original surface and is addressed by the supplement below.

The correction makes canonical active `land_listing_links` authoritative even
when a malformed linked listing says `house`; it still immediately rejects a
dedicated-Land marker, rejects an unlinked `plot`/`land` row while deferred,
and allows an unlinked ordinary residential row. Known policy rejections map to
a safe generic-workflow precondition. Unexpected Land-link/database lookup
failures remain internal operational errors and retain their cause for
diagnostics. The authority-wrapped 10-file run passed **144 tests**, the
explicit client run passed **48 tests**, the static authority suite passed
**293 tests**, `pnpm check` passed, targeted ESLint had no errors, and
`git diff --check` passed. The retained target remained exact-worktree-owned,
schema-congruent at `0090`, with no incomplete attempts.

### Generic Developer-Land supplement (2026-09-15)

A later source-and-counterexample audit found a separate generic Developer
escape route: `developments.development_type = 'land'` remained accepted by
the generic authoring API, and a retained row in the `approved`/`isPublished`
state satisfied the central generic public-development predicate. This was not
the specialist Land record model and does not invalidate the earlier direct
Land correction; it was a distinct legacy-capable route that needed the same
first-cohort disposition.

Commit [`62ec0eed`](https://github.com/Doscoding187/real_estate_portal/commit/62ec0eede830e96eafc69dfd9e4f42a9645aa1ee)
adds the immutable deferred-Land policy to generic Development authoring,
editing, routine availability, submission, approval, and publication
transitions. Its central public-eligibility evaluator and SQL predicate exclude
generic Land rows from public detail, catalogue listing, and bounded search.
Existing rows are retained; neither the patch nor its test fixture performs a
migration, deletes Land data, changes Land geography/classification authority,
enables payment or entitlement, or creates an alternate Land lifecycle.

The focused authority-wrapped run passed **3 files / 42 tests** on exact target
fingerprint `a560e9f2971e7676…`. It models a retained generic Land row with
legacy public flags, then proves new authoring, editing, availability changes,
and platform publication fail closed without state mutation; public detail,
list, and search omit that row. The evaluator’s first failing test established
the pre-fix counterexample, after which the same evaluator, source contract,
and persisted integration suite passed. `pnpm check`, the **35-file / 294-test**
database-authority static suite, targeted ESLint with zero errors (existing
warnings only), `git diff --check`, and a final authority status all passed.
The target stayed exact-worktree-owned, schema-congruent at migration head
`0090_retire_disconnected_boost_campaigns.sql`, with no incomplete attempts.

This is local task-branch containment evidence, not Land integration,
commercial activation, hosted verification, or production proof. The existing
...
has no approved live onboarding route. The phrase “first cohort” below means
the controlled **pre-payment onboarding cohort**; an isolated paid-entitlement
...
added the first authority-bound Chromium acceptance for standalone-agent
preparation. Commit [`e9909ab6`](https://github.com/Doscoding187/real_estate_portal/commit/e9909ab6be90323a881f07a267777c64b1e075d3)
extends that governed acceptance to a newly registered agency owner and corrects
two misleading pre-payment presentations: agency setup no longer says it will
issue an invoice, and a `pending_payment` commercial preference no longer shows
an invoice-ready action while activation is disabled. The correction is a UI
projection only; it preserves the canonical subscription state and the server's
fail-closed payment and entitlement controls.

The acceptance deliberately does not reuse older Playwright configurations that
hard-code another local database. `playwright.prepayment-onboarding.config.ts`
starts the repository's exact-target `mvp-local-runtime.mts` API process at
`localhost:5000` and a local Vite browser shell at `localhost:5177`. The
recorded command invokes it through `pnpm test:browser:authority`, which resolves
and passes the governed authority context to its child processes.
...
profile, selected Sandton from the canonical location autocomplete, completed
setup, reached `/listings/create`, and reloaded that workspace. The UI continued
...
the agency shell and preparation banner. The setup explicitly stated that it
does not issue an invoice, request payment, or activate publishing. The
workspace presented “Prepare inventory” and did not present “Your invoice is
ready.”

The test queried the same authority-owned target after the UI actions. For the
agent, it asserted a verified agent role, display name, phone, bio, no agency
affiliation, and one canonical `suburb:<id>` Sandton coverage identity with its
display label. For the agency owner, it asserted a verified `agency_admin`, a
persisted agency and branding record with its selected geography, a canonical
`pending_payment` subscription with a valid plan selection, and **zero**
`billing_invoices`. This is evidence that normal pre-payment setup saves a
...
### Shared public advertising and Developer preparation-entry truthfulness — 2026-09-15

The Agent and Agency landing corrections did not fully close the anonymous
commercial funnel. The routable `/advertise` and `/advertise/sell` gateways
still described 90-Day Launch Access, manual EFT, finance-verified activation,
public discovery, enquiries, and paid commercial participation. The public
Developer route also unconditionally loaded its commercial catalog and offered
Developer Launch Access, an invoice request, and a fixed paid term. Those claims
contradicted the same normal `preparation_only` state.

Commit [`234ca6e4`](https://github.com/Doscoding187/real_estate_portal/commit/234ca6e4f2c4833f5784300a1edb9ee016f2c386)
retains each catalog-driven commercial page behind the existing separately
authorized `COMMERCIAL_ACTIVATION_STATE.enabled` boundary. Normal `/advertise`
and `/advertise/sell` now show only role-specific preparation paths, and the
homepage professional entry names preparation rather than commercial access.
Normal Developer entry starts the real role-specific registration target and
describes only Developer account creation, organisation identity/review, and
private development drafts. It does not load the commercial catalog or advertise
a price, invoice, manual-EFT payment, fixed paid term, public project
presentation, or paid commercial participation.

The underlying Developer route and existing focused tests support the stated
preparation boundary: email verification returns a Developer to
...
### Direct commercial-plans route containment — 2026-09-15

The public-entry correction had covered `/advertise`, `/advertise/sell`, and
the role-specific landing routes, but the mounted direct
`/subscription-plans` route still rendered canonical paid product names,
prices, fixed terms, and capability descriptions in normal
`preparation_only` runtime. Its button label said “Prepare workspace,” but
that did not make the paid catalogue an accurate public proposition while the
same runtime forbids invoices, payment, entitlement mutation, and publishing.

Commit
[`32224b0a`](https://github.com/Doscoding187/real_estate_portal/commit/32224b0ada4e34a1e9222d702d8fa46963bc88f2)
places that catalog presentation behind the existing
`COMMERCIAL_ACTIVATION_STATE.enabled` decision. The normal direct route now
shows the canonical preparation notice and Agent, Agency, and Developer
preparation choices only; it does not load the commercial catalog or show paid
prices, terms, invoice actions, or manual-EFT claims. The retained catalog page
remains available only to a separately authorised future enabled runtime.

Focused client coverage passed **1 file / 2 tests**, including the assertion
that normal runtime does not call the commercial catalog and that the retained
commercial page still renders only when explicitly selected. The governed
Chromium pre-payment acceptance passed **8 tests**; its shared public-entry
case now opens the real `/subscription-plans` route and proves the preparation
boundary there. `pnpm check`, targeted zero-error ESLint/Prettier,
...
This is a public presentation correction. It does not alter the canonical
catalog, activate commercial operations, issue invoices, create payment or
entitlement state, change deployment/provider settings, or prove a paid cohort.

### Authenticated Developer plans containment — 2026-09-15

The authenticated `/developer/plans` route was a separate commercial
presentation escape. In normal `preparation_only` runtime it loaded the
Developer catalog, subscription projection, and invoice mutation wiring, then
displayed paid product names, prices, terms, and capabilities. The Developer
workspace's inactive-access card also offered an “Activate Launch Access”
button that led to that unavailable commercial page.

Commit
[`ada578a2`](https://github.com/Doscoding187/real_estate_portal/commit/ada578a2ae2226df0497bb8769911fa668b85149)
retains the catalog-driven page as an explicit commercial component behind the
existing future enabled-state decision. The normal authenticated route now
shows the preparation notice and only private-development actions; it does not
load a catalog, subscription projection, or invoice mutation. The inactive
workspace card now directs the Developer to manage private developments rather
than suggesting unavailable activation.

Focused client coverage passed **2 files / 2 tests**, proving both direct-route
containment and the inactive-workspace navigation outcome. `pnpm check`,
targeted zero-error ESLint/Prettier, `pnpm test:db-authority:static` (**35
files / 295 tests**), `git diff --check`, and final `pnpm db:authority:status`
passed on the exact worktree-owned target at migration head `0090` with no
incomplete attempts.

This correction preserves the existing entitlement policy and future
commercial implementation. It neither makes a commercial request nor changes
...
### Authenticated Developer subscription-route containment — 2026-09-16

The authenticated `/developer/subscription` route and its
`/developer/settings/subscription` alias remained normal-runtime commercial
presentation escapes. In `preparation_only` runtime, the reachable panel mounted
subscription and billing-workspace data plus a payment-proof mutation, then
rendered billing history, invoice, manual-EFT, and payment controls. A pending
Developer was also redirected away from the safe route before it could continue
preparing its organisation and private inventory.

Commit
[`bb89b2b9`](https://github.com/Doscoding187/real_estate_portal/commit/bb89b2b92ff421e54e9fe864e71112b2947b05f6)
keeps the existing billing implementation as `CommercialDeveloperBillingPanel`
behind the existing separately authorised enabled-state decision. The normal
routes now show only the preparation notice, organisation-readiness context,
and private-development actions; they do not mount the commercial data hooks or
payment mutation. The pending-profile route guard admits these safe routes, so
an otherwise verified Developer can continue private preparation rather than
being redirected to the dashboard.

Focused client coverage passed **4 files / 5 tests**, covering direct panel
containment, both pending-profile route aliases, the retained plans boundary,
and inactive workspace state. The governed Chromium pre-payment suite passed
**8 tests**. Its real pending-Developer flow registers, verifies, submits
organisation intake, opens `/developer/subscription`, confirms billing history,
Developer invoice, and manual-EFT controls are absent, then creates and resumes
a private development draft. `pnpm check`, targeted zero-error ESLint,
`pnpm test:db-authority:static` (**35 files / 295 tests**), `git diff --check`,
and final `pnpm db:authority:status` passed on the exact task-owned target at
migration head `0090` with no incomplete attempts.

This is a normal-runtime presentation and navigation correction only. It does
not issue an invoice, accept a payment proof, mutate entitlement state, publish
a development, access a provider, or change a protected environment. It is
local task-branch evidence and does not authorize a paid Developer cohort.

### Legacy public activation-route containment — 2026-09-16

A mounted anonymous `/activation` route still lazy-loaded the retired
`ActivationGate` screen in normal `preparation_only` runtime. That page told an
anonymous stakeholder that an agency profile was live and offered active
listing, CRM-import, and marketplace-oriented actions. Its terminal dashboard
navigation did not make that unsupported public presentation truthful.

Commit
[`a0ceea70`](https://github.com/Doscoding187/real_estate_portal/commit/a0ceea70940e3ee3f84b7118a0470e3b4c7b4631)
removes the stale screen mount and preserves the inbound URL as a safe redirect
to `/advertise`. The existing preparation entry explains the supported Agent,
Agency, and Developer paths without claiming commercial activation, public
inventory, CRM import, or marketplace access. The retired component is not
repurposed as an enabled-commercial path.

Focused source-contract coverage passed **1 file / 6 tests**. The governed
Chromium check passed the specific legacy-route flow, and the final full
governed pre-payment browser suite passed **8 tests**. `pnpm check`,
`pnpm test:db-authority:static` (**35 files / 295 tests**), `git diff --check`,
and final `pnpm db:authority:status` passed on the exact task-owned target at
migration head `0090` with no incomplete attempts.

This is a normal-runtime public-entry containment correction only. It does not
...
### Direct administrator review-route authority containment — 2026-09-16

The dedicated `/admin/review/:id` route mounted the reviewer-only
`AdminPropertyReview` workspace directly while the adjacent administrative
routes and dashboard registry use the shared `RequireRole` boundary. The
listing APIs still require authentication and enforce content custody; the
approve and reject mutations separately require `super_admin`. This review did
not establish a server-side authorisation bypass. It did establish an
inconsistent client authority boundary: a direct anonymous URL could mount the
private review interface before the expected administrative role check and
issue its dependent protected read.

Commit
[`ddbb7e58`](https://github.com/Doscoding187/real_estate_portal/commit/ddbb7e5893d2bd65361103f227887fd4c1f2e9b1)
places that retained direct route behind the established `super_admin`
`RequireRole` guard with contextual sign-in. An unauthenticated URL now lands
on sign-in with the exact `/admin/review/:id` return target, including safe
query state, and the review workspace does not mount first. The established
server custody and decision checks remain unchanged.

Focused source and role-boundary coverage passed **3 files / 19 tests**. The
specific governed Chromium direct-URL check passed, and the final full
governed pre-payment suite passed **10 tests**. `pnpm check`, `git diff
--check`, and `pnpm test:db-authority:static` (**35 files / 295 tests**) passed.
Targeted ESLint had no errors; it retained six pre-existing `App.tsx` warnings
and the existing ignored-E2E-file notice. Targeted Prettier passed the new test
and E2E file; `App.tsx` retains a pre-existing unrelated formatting warning at
its assisted-onboarding import. The final `pnpm db:authority:status` confirmed
the exact task-owned target at migration head `0090` with no incomplete
attempts.

This is a client route-boundary correction only. It does not change listing
custody, reviewer privileges, payment, entitlement, publication, providers,
database schema or data, deployment, or any protected environment.
...
4. **L1 — integrate and independently verify public preparation and administrative-route authority (LRC-ADVERTISE-001; LRC-AGENT-001; LRC-AGY-001; LRC-DEV-001; LRC-AUTH-001):** `234ca6e4` makes `/advertise`, `/advertise/sell`, the homepage professional entry, and the public Developer landing agree with normal `preparation_only` runtime; `32224b0a` closes the direct `/subscription-plans` paid-catalog escape; `e1ccc2c8` closes the analogous authenticated Agent package page and shared unavailable-activation CTA escape; `d48544b9` closes the authenticated Agency Billing route; `ada578a2` closes the authenticated Developer plans escape and inactive-workspace activation CTA; `bb89b2b9` closes the authenticated `/developer/subscription` and `/developer/settings/subscription` billing-route escapes; `a0ceea70` redirects the stale public `/activation` route to preparation rather than mounting false live-agency, listing, or CRM-import claims; `3ad9b1f5` routes the legacy Developer authoring URLs through the canonical authenticated workspace while preserving safe draft/edit return targets; `ddbb7e58` applies the same `super_admin` client boundary to the retained direct `/admin/review/:id` workspace while preserving server custody and reviewer checks; `217aa1df` makes all reachable profile-completion locks describe actual private preparation rather than unavailable activation; `7c939831` and `02788ff2` retain the role-specific Agent and Agency corrections; `9cd6d3e3` browser-proves the pending Developer organisation/private-draft preparation slice; `589135ce` proves the separate authorised approval-to-professional-presence boundary preserves `missing_launch_access` and an empty public project set. Every paid-operation variation remains separate. Do not advertise paid cohorts before integration and the relevant hosted verification.
...
`cf1f93e6`, canonical private-work membership correction `5f37d3d1`, canonical agency-inventory scope correction `0eaf2cc8`, canonical operational-workspace membership correction `32ce7524`, canonical viewing/deal workspace membership correction `47e059ef`, canonical Agent Home workspace membership correction `a0fc9e8b`, `af9fd6f2`, `16a23cd9`, Goal 4 evidence/fixture correction
...
acceptance `9cd6d3e3`, legacy public activation-route containment `a0ceea70`, legacy Developer authoring-route containment `3ad9b1f5`, direct administrator review-route authority containment `ddbb7e58`, and Agent preparation-lock wording correction `217aa1df`; the review packet,
