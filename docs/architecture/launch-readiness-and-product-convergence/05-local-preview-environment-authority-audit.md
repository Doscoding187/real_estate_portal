...
5. The normal bootstrap is not an audit command because it can create a missing link and start/migrate/seed local MySQL. By contrast, the mandatory status command is non-mutating for every target classification: it inspects and classifies first, returns a sanitized `not queried` result for every unapproved target, and permits only an approved local/test target to receive the read-only migration-ledger query.
...
| Database-authority status | `pnpm db:authority:status` is non-mutating for every target classification: it reads authority metadata and reports a sanitized classification before connection. Unknown, malformed, unapproved, remote, staging, and production-like targets return `not queried`; only an approved local/test target may open a MySQL connection for the read-only migration-ledger query. | **Run in Stage 2A review closure from the control worktree:** manifest validation passed; central file found with safe permissions; worktree state was `conflicting-file`; required local states were configured; target classified approved local; the migration ledger was reachable. It performed only a read-only ledger `SELECT`. | Verified mandatory orientation |
...
| Current trust level | **Partial:** structure, local-user ownership, `0600` mode, required database-authority variable states, and sanitized target classification were verified by the successful status run. The target was approved local and the migration ledger reachable at that time. Worktree linkage remains conflicted; complete application sufficiency and hosted deployment bindings remain unproven. |
...
Before execution, `AGENTS.md`, `package.json`, `scripts/databaseAuthorityStatus.ts`, `scripts/localEnvironmentAuthority.ts`, and the database-authority entry contract were inspected. `pnpm db:authority:status` is the mandatory non-mutating orientation command for every target classification: it validates the manifest, inspects central-file and link metadata, checks required database-authority variable states, and produces a sanitized target classification before any connection attempt. Unknown, malformed, unapproved, remote, staging, and production-like targets are reported without connection. Running status does not approve a target. Only an approved local/test target may proceed to the read-only migration-ledger `SELECT`; the command never runs migrations, seeds, creates or replaces links, changes permissions, or starts/stops MySQL or Docker.
...
| `scripts/databaseAuthorityStatus.ts` | Non-mutating for every target classification: validates the manifest, reads central/link state, checks required database-authority variable states, and classifies the target. Unapproved classifications return `not queried` before `mysql.createConnection`; only approved local/test targets query the migration ledger. | Inspected and run during review closure. The observed approved-local path performed one read-only local ledger query and returned sanitized authority status. |
