...
5. The normal bootstrap is not an audit command because it can create a missing link and start/migrate/seed local MySQL. By contrast, the mandatory status command is non-mutating for every target classification: it classifies the target internally before deciding whether connection is permitted, returns an unqueried ledger state without connection for every unapproved target, and permits only an approved local/test target to receive the read-only migration-ledger query before it prints consolidated final status output.
...
| Database-authority status | `pnpm db:authority:status` is non-mutating for every target classification. It classifies the target internally before deciding whether connection is permitted. Unknown, malformed, unapproved, remote, staging, and production-like targets return with the ledger unqueried and without connection; only an approved local/test target may open a MySQL connection for the read-only migration-ledger query. For an approved target, that query occurs before the command prints its final consolidated status output. | **Run in Stage 2A review closure from the control worktree:** manifest validation passed; central file was accessible and the helper reported safe permission mode; worktree state was `conflicting-file`; required local states were configured; target classified approved local; the migration ledger was reachable. It performed only a read-only ledger `SELECT`. | Verified mandatory orientation |
...
| Current trust level | **Partial:** evidence is deliberately separated below so future operators do not treat the status command as a complete filesystem or structural validator. |
| **Verified by status execution** | Authority manifest validation; central-path accessibility; the helper's followed-target permission-mode result; required database-authority variable states; control-worktree link classification; sanitized approved-local classification at inspection time; migration-ledger reachability at inspection time; and no mutation in the observed path. |
| **Verified by separate Stage 2A filesystem/structure inspection** | Local-user owner and group; the central path being a regular file rather than a symlink; `0600` path metadata; timestamp; complete nine-name inventory; no duplicate names; and no malformed assignment lines. These were not asserted by the status helper itself. |
| **Still unproven** | Compliant control-worktree linkage; complete application sufficiency of the nine-variable central file; disposition of the 69 control-only names; Railway/Vercel bindings; and canonical-preview readiness. |
...
Before execution, `AGENTS.md`, `package.json`, `scripts/databaseAuthorityStatus.ts`, `scripts/localEnvironmentAuthority.ts`, and the database-authority entry contract were inspected. The execution order is: manifest validation; central-path and worktree-link inspection; required database-authority variable-state inspection; internal target classification; connection-eligibility decision; migration-ledger call or unqueried-ledger result; then final `console.log` status output. `pnpm db:authority:status` is therefore the mandatory non-mutating orientation command for every target classification, but it does not promise that an operator sees final output before an approved connection attempt. Unknown, malformed, unapproved, remote, staging, and production-like targets return without connection and are later reported with the ledger unqueried. Running status does not approve a target. Only an approved local/test target may proceed to the read-only migration-ledger `SELECT`; the command never runs migrations, seeds, creates or replaces links, changes permissions, or starts/stops MySQL or Docker.
...
| `scripts/databaseAuthorityStatus.ts` | Non-mutating for every target classification: validates the manifest, inspects central/link state and required database-authority variable states, then classifies and gates the target internally. Unapproved classifications return the ledger `not queried` before `mysql.createConnection`; only approved local/test targets query the migration ledger, and final console output follows that call. It does not validate owner, path type, duplicate names, or full assignment syntax. | Inspected and run during review closure. The observed approved-local path performed one read-only local ledger query and then returned sanitized authority status. |
...
The authority library has explicit preservation protections, and the existing status command already provides non-mutating classification without connecting to unapproved targets. The workflow still lacks a dry-run/plan for one worktree, a safe name-only reconciliation report, automatic worktree-link integration, and drift detection across registered worktrees. The new worktree proves no automatic integration currently exists. A future diagnostic must report status-command safety separately from database-connection eligibility and must not treat the existing nine-variable central file as the complete application authority until the Stage 2B contract is approved.
...
| --- | --- | --- | --- | --- |
| 1 | Stage 2B: define the complete variable-by-variable local-environment authority contract and implement only non-mutating diagnostics. | Repository source only | No link creation; name-only/sanitized output; the status path remains non-mutating for every target classification, while any migration-ledger query remains limited to approved local/test targets; focused tests isolated from real machine files. | Approve contract and diagnostic design. |
