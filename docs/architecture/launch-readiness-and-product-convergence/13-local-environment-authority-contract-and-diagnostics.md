...
The focused suite `server/__tests__/localEnvironmentAuthorityStatus.test.ts` passes 35 tests covering:
...
- canonical required-value validation, unknown/deprecated/unknown-pending exit status, runtime dotenv comment semantics, and noncanonical symlink non-following;
- exact raw database pathname validation, including rejection of percent-encoded path variants;
