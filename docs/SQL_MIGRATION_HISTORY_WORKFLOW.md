...
Baseline mode requires an empty custom ledger and is permitted only with `NODE_ENV=development` or `test`; staging and production baseline attempts fail. For every file through the supplied boundary it checks structural schema witnesses (created tables, added columns, indexes and foreign keys) before writing the committed checksum. It fails on a missing witness or a migration for which no structural witness can be safely derived; add a migration-specific verifier instead of inserting a history row. Baseline mode is deliberately not part of the normal path, and the local wrapper prints the target without credentials before it can run.
