import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, int, varchar, text, json, mysqlEnum, timestamp, decimal, date, datetime, mysqlView } from "drizzle-orm/mysql-core"
import { sql } from "drizzle-orm"

export const activities = mysqlTable("activities", {
	id: int().autoincrement().notNull(),
	developerId: int("developer_id").notNull().references(() => developers.id),
...
export const boostCampaigns = mysqlTable("boost_campaigns", {
	...
	partnerId: varchar("partner_id", { length: 36 }).notNull().references(() => explorePartners.id),
...
export const bundlePartners = mysqlTable("bundle_partners", {
	bundleId: varchar("bundle_id", { length: 36 }).notNull().references(() => marketplaceBundles.id),
...
(table) => [
	index("idx_city_slug").on(table.slug),
]);
...
export const contentQualityScores = mysqlTable("content_quality_scores", {
	...
	lastCalculatedAt: timestamp("last_calculated_at", { mode: 'string' }).default('CURRENT_TIMESTAMP'),
},
(table) => [
...
export const contentTopics = mysqlTable("content_topics", {
	contentId: varchar("content_id", { length: 36 }).notNull(),
	topicId: varchar("topic_id", { length: 36 }).notNull().references(() => topics.id),
...
export const developerSubscriptionLimits = mysqlTable("developer_subscription_limits", {
	id: int().autoincrement().notNull(),
	subscriptionId: int("subscription_id").notNull().references(() => developerSubscriptions.id),
...
export const developmentApprovalQueue = mysqlTable("development_approval_queue", {
	id: int().autoincrement().notNull(),
	developmentId: int("development_id").notNull().references(() => developments.id),
	submittedBy: int("submitted_by").notNull().references(() => users.id, { onDelete: "cascade" } ),
...
export const developments = mysqlTable("developments", {
	...
	developerBrandProfileId: int("developer_brand_profile_id").references(() => developerBrandProfiles.id, { onDelete: "set null" } ).references(() => developerBrandProfiles.id, { onDelete: "set null" } ),
	...
	status: mysqlEnum(['launching-soon','selling','sold-out']).default('launching-soon').notNull(),
	legacyStatus: mysqlEnum("legacy_status", ['planning','under_construction','completed','coming_soon','now-selling','launching-soon','ready-to-move','sold-out','phase-completed','new-phase-launching','pre_launch','ready']),
	constructionPhase: mysqlEnum("construction_phase", ['planning','under_construction','completed','phase_completed']),
	...
	floors: int(),
	transactionType: mysqlEnum("transaction_type", ['for_sale','for_rent','auction']).default('for_sale').notNull(),
...
(table) => [
	index("idx_quota_type").on(table.contentType),
]);
...
(table) => [
	index("idx_suburb_slug").on(table.slug),
]);
...
export const unitTypes = mysqlTable("unit_types", {
	...
	auctionStartDate: datetime("auction_start_date", { mode: 'string'}),
	auctionEndDate: datetime("auction_end_date", { mode: 'string'}),
	auctionStatus: mysqlEnum("auction_status", ['scheduled','active','sold','passed_in','withdrawn']).default('scheduled'),
},
(table) => [
...
}).algorithm("undefined").sqlSecurity("definer").withCheckOption("cascaded").as(sql`SELECT 1 AS \`sourceType\`,1 AS \`sourceId\`,1 AS \`developmentId\`,1 AS \`province\`,1 AS \`created_at\`,1 AS \`locationId\`,1 AS \`cityLocationId\`,1 AS \`suburbLocationId\`,1 AS \`priceAmount\`,1 AS \`areaM2\`,1 AS \`offerKind\`,1 AS \`isActive\``);