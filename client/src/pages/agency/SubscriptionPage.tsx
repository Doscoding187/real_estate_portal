...
import { CheckCircle, AlertCircle, Download, Upload, Shield, ArrowLeft } from 'lucide-react';
...
export default function AgencySubscriptionPage() {
  ...
  const { data: subscription, isLoading: isLoadingSub } =
    trpc.subscription.getCurrentSubscription.useQuery(undefined, { enabled: agencyReady });
  ...
  const { data: bankingDetails } = trpc.subscription.getBankingDetails.useQuery(undefined, {
    enabled: agencyReady,
  });
          ...
          <>
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                {status?.agency?.name ? (
                  <p className="text-xs text-slate-400 mt-1">{status.agency.name}</p>
                ) : null}
              </div>
            </div>
