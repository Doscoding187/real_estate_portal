...
export default function CommercialOffice() {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <section
        aria-label="Commercial Office filters"
        className="grid gap-3 rounded border bg-slate-50 p-4 md:grid-cols-4"
      >
        <Input
          aria-label="Location"
          value={location}
        ...
        />
        <label className="flex items-center gap-2 text-sm" htmlFor="commercial-backup-power">
          <Checkbox
            id="commercial-backup-power"
            aria-label="Backup power"
            checked={backupPower}
            onCheckedChange={checked => setBackupPower(checked === true)}
          />
          <span>Backup power</span>
        </label>
        <label className="flex items-center gap-2 text-sm" htmlFor="commercial-backup-water">
          <Checkbox
            id="commercial-backup-water"
            aria-label="Backup water"
            checked={backupWater}
            onCheckedChange={checked => setBackupWater(checked === true)}
          />
          <span>Backup water</span>
        </label>
        <label className="flex items-center gap-2 text-sm" htmlFor="commercial-fibre">
          <Checkbox
            id="commercial-fibre"
            aria-label="Fibre"
            checked={fibreConnectivity}
            onCheckedChange={checked => setFibreConnectivity(checked === true)}
          />
          <span>Fibre</span>
        </label>
      </section>
      ...
      {!unsupportedLocationScope && !results.isLoading && totalResults === 0 ? (
        <p>No published Office spaces match these requirements.</p>
      ) : null}
