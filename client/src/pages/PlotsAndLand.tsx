...
export default function PlotsAndLand() {
  ...
  const handoffError = initial.get('searchError');
  ...
  const locationId = initial.get('locationId') || undefined;
  const locationIds = initial.getAll('locationIds');
  const searchAreaId = initial.get('searchAreaId') || undefined;
  ...
  }, [classification, city, province, minPrice, maxPrice, minSize, maxSize, handoffError, locationId, searchAreaId, locationIds.join('|')]);
  const results = trpc.landPublic.search.useQuery(queryInput);

  return <main className="mx-auto max-w-6xl space-y-6 p-4 md:p-6"><header><h1 className="text-3xl font-semibold">Plots &amp; Land</h1><p className="mt-2 text-slate-600">Find land. Understand the land. Verify what matters.</p></header>{handoffError === 'unsupported-location-scope' && <p role="alert" className="rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">That location scope is not available for Land yet. Choose a canonical province, city, suburb, sibling locations, or an approved Search Area.</p>}{results.error && <p role="alert" className="rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{results.error.message}</p>}<section aria-label="Land search filters" className="grid gap-3 rounded border bg-white p-4 sm:grid-cols-2 lg:grid-cols-4"><label className="grid gap-1 text-sm"><span>City</span><input className="rounded border p-2" placeholder="City" value={city} onChange={event => setCity(event.target.value)} /></label><label className="grid gap-1 text-sm"><span>Province</span><input className="rounded border p-2" placeholder="Province" value={province} onChange={event => setProvince(event.target.value)} /></label><label className="grid gap-1 text-sm"><span>Land type</span><select className="rounded border p-2" value={classification} onChange={event => setClassification(event.target.value as LandPublicClassification | '')}><option value="">Any land type</option>{LAND_PUBLIC_CLASSIFICATIONS.map(value => <option key={value} value={value}>{LAND_CLASSIFICATION_LABELS[value]}</option>)}</select></label><label className="grid gap-1 text-sm"><span>Minimum price (R)</span><input className="rounded border p-2" type="number" min="1" value={minPrice} onChange={event => setMinPrice(event.target.value)} /></label><label className="grid gap-1 text-sm"><span>Maximum price (R)</span><input className="rounded border p-2" type="number" min="1" value={maxPrice} onChange={event => setMaxPrice(event.target.value)} /></label><label className="grid gap-1 text-sm"><span>Minimum extent (m²)</span><input className="rounded border p-2" type="number" min="1" value={minSize} onChange={event => setMinSize(event.target.value)} /></label><label className="grid gap-1 text-sm"><span>Maximum extent (m²)</span><input className="rounded border p-2" type="number" min="1" value={maxSize} onChange={event => setMaxSize(event.target.value)} /></label></section><section aria-live="polite" className="grid gap-4 md:grid-cols-3">{results.data?.map(item => <Link key={item.listingId} href={item.href} className="rounded border p-4 transition hover:border-slate-500"><p className="font-semibold">{item.title}</p><p>{LAND_CLASSIFICATION_LABELS[item.classification as keyof typeof LAND_CLASSIFICATION_LABELS] || item.classification} · {Number(item.extentM2).toLocaleString()} m²</p><p>{item.city}, {item.province}</p><p>{item.intendedUse || 'Land opportunity'}</p><p className="mt-2 font-medium">R {Number(item.askingPrice).toLocaleString()}</p><p className="text-sm text-slate-600">{item.passport.trustState?.replaceAll('_', ' ') || 'Listed with disclosures'}</p></Link>)}</section>{!results.isLoading && !results.error && !results.data?.length && <p className="rounded border border-dashed p-6 text-slate-600">No eligible Land listings match these exact filters.</p>}</main>;
}
