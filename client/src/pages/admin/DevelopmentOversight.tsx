...
export default function DevelopmentOversight() {
  ...
  const { data: pendingDevelopments, isLoading, refetch } = trpc.admin.adminListPendingDevelopments.useQuery();
  ...
  // Filter data
  const filteredData = pendingDevelopments?.filter(dev => 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
             <GlassCard className="border-white/40 shadow-sm">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium text-slate-500 uppercase tracking-wider">Pending Review</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="text-3xl font-bold text-orange-600">{pendingDevelopments?.length || 0}</div>
                </CardContent>
            </GlassCard>
            <GlassCard className="border-white/40 shadow-sm">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium text-slate-500 uppercase tracking-wider">Fast-Track Ratio</CardTitle>
                </CardHeader>
                <CardContent>
             ...
             <GlassCard className="border-white/40 shadow-sm">
                <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium text-slate-500 uppercase tracking-wider">Avg Manual Review</CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <GlassCard className="border-white/40 shadow-sm overflow-hidden">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow>
                     <TableCell colSpan={6} className="h-24 text-center">Loading pending developments...</TableCell>
                  </TableRow>
                ) : filteredData.length === 0 ? (
                   <TableRow>
                     <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">No pending approvals found.</TableCell>
                  </TableRow>
                ) : (
                  filteredData.map((dev) => (
                    <TableRow key={dev.id} className="hover:bg-slate-50 transition-colors">
                      <TableCell>
                       ...
                       <TableCell>
                           <Badge variant="outline" className="capitalize">{dev.developmentType}</Badge>
                       </TableCell>
                       <TableCell>
                       ...
                       <TableCell className="text-right">
                           <Button size="sm" onClick={() => handleReview(dev)} className="bg-blue-600 hover:bg-blue-700">
                               Review Application
                           </Button>
                       </TableCell>
                    </TableRow>
        ...
        <Dialog open={isActionDialogOpen} onOpenChange={(open) => !open && closeDialog()}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                {selectedDevelopment && (() => {
                    ...
                    return <DevelopmentReviewContent development={selectedDevelopment} reviewAction={reviewAction} setReviewAction={setReviewAction} confirmAction={confirmAction} isMutating={approveMutation.isPending || rejectMutation.isPending || requestChangesMutation.isPending} actionReason={actionReason} setActionReason={setActionReason} />;
                })()}
            </DialogContent>
        </Dialog>
      </div>
...
function DevelopmentReviewContent({ 
    ...
    const { data: auditLogs, isLoading: isLoadingHistory } = trpc.admin.getDevelopmentAuditLogs.useQuery(
    ...
    return (
        <div className="space-y-6 py-4">
            ...
            <div className="flex items-start gap-4 p-4 bg-slate-50 rounded-lg border border-slate-100">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            ...
                            <Badge variant="outline">Submitted: {new Date(development.submittedAt).toLocaleDateString()}</Badge>
                        </div>
                    </div>
            </div>
            ...
            {/* Description */}
            <div>
                <h4 className="font-semibold mb-2 text-sm text-slate-700 uppercase tracking-wide">Description</h4>
            ...
            {/* Amenities */}
            <div>
                <h4 className="font-semibold mb-2 text-sm text-slate-700 uppercase tracking-wide">Amenities</h4>
                <div className="flex flex-wrap gap-2">
                    {amenities.map((amenity: string, idx: number) => (
                        <Badge key={idx} variant="secondary" className="font-normal">{amenity}</Badge>
                    ))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                           ...
                           <div key={item.id} className="flex items-center space-x-2">
                                ...
                                />
                                <Label htmlFor={`check-${item.id}`} className="text-sm cursor-pointer text-slate-700 font-normal">{item.label}</Label>
                           </div> 
                        ))}
                    </div>
                </div>
            ...
            <div className="pt-4 border-t border-slate-100">
                    {!reviewAction ? (
                        <div className="flex gap-4 justify-between items-center">
                            <Button variant="link" className="text-slate-500" onClick={() => window.open(`/development/${development.id}`, '_blank')}>
                                <Eye className="w-4 h-4 mr-2" /> View Full Preview
                            </Button>
                             <Button variant="ghost" className="text-slate-500" onClick={() => setShowHistory(!showHistory)}>
                                <History className="w-4 h-4 mr-2" /> {showHistory ? 'Hide History' : 'View History'}
                            </Button>
                            <div className="flex gap-2">
                                <Button variant="outline" onClick={() => setReviewAction('request_changes')} className="gap-2 text-amber-600 hover:text-amber-700 hover:bg-amber-50 border-amber-200">
                                    <MessageSquare className="w-4 h-4" /> Request Changes
                                </Button>
                                <Button variant="destructive" onClick={() => setReviewAction('reject')} className="gap-2">
                                    <Ban className="w-4 h-4" /> Reject
                                </Button>
                                <Button className="bg-green-600 hover:bg-green-700 gap-2" onClick={() => setReviewAction('approve')}>
                        ...
                        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                            ...
                            <p className="text-sm text-green-700 mb-4">
                                This will immediately publish the development to the public portal and notify the developer.
                            </p>
                            
                            <div className="flex gap-2 justify-end">
                                <Button variant="ghost" onClick={() => setReviewAction(null)} disabled={isMutating}>Cancel</Button>
                                <Button className="bg-green-600 hover:bg-green-700" onClick={() => confirmAction(checklist)} disabled={isMutating}>
                        ...
                        <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                            ...
                            <p className="text-sm text-amber-700 mb-4">
                                The developer will be notified to make updates. Status will revert to 'Changes Requested'.
                            </p>
                            <Textarea 
                                value={actionReason}
                            ...
                            <div className="flex gap-2 justify-end">
                                <Button variant="ghost" onClick={() => setReviewAction(null)} disabled={isMutating}>Cancel</Button>
                                <Button className="bg-amber-500 hover:bg-amber-600 text-white" onClick={() => confirmAction()} disabled={isMutating}>
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 max-h-60 overflow-y-auto space-y-4">
                        {isLoadingHistory ? (
                            ...
                            auditLogs.map((log) => (
                                <div key={log.id} className="flex gap-3 text-sm">
                                    ...
                                    <div className="flex-1 pb-2">
                                        <div className="font-medium text-slate-900">
                                            ...
                                            <span className="text-slate-500 font-normal"> - {log.action.replace('UPDATE_DEVELOPMENT', 'Update')}</span>
                                        </div>
                                        {log.metadata && (
                                            <div className="text-slate-600 mt-1 bg-white p-2 rounded border border-slate-200 inline-block max-w-full">
                                                ...
                                                {(() => {
                                                    try {
                                                       ...
                                                       if (meta.action === 'request_changes') return <span className="text-amber-700">Changes Requested: "{meta.feedback}"</span>;
                                                       if (meta.action === 'reject') return <span className="text-red-700">Rejected: "{meta.reason}"</span>;
                                                       if (meta.action === 'approve') return <span className="text-green-700">Approved</span>;
                             ...
                             <div className="text-center text-sm text-slate-500 py-4 italic">No history found.</div>
                        )}
                    </div>
                </div>
            )}
