    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => setLocation('/admin/marketing')}>
            Cancel
          </Button>
        </div>
      </div>
        ...
        <div className="relative flex justify-between">
          {STEPS.map(step => (
            <div key={step.id} className="flex flex-col items-center gap-2">
              ...
              <span
                className={`text-xs font-medium ${step.id === currentStep ? 'text-blue-600' : 'text-slate-500'}`}
              >
                {step.name}
              </span>
            </div>
      ...
      <Card className="min-h-[400px]">
        <CardContent className="p-6">
          ...
          {currentStep === 8 && campaignId && (
            <Step8Billing data={formData} campaignId={campaignId} onBack={handleBack} />
          )}
        </CardContent>
      </Card>
    </div>
