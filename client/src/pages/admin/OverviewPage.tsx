    ...
    <div className="space-y-8 max-w-7xl mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {pulseMetrics.map((metric, i) => (
          <Card key={i} className="py-6 px-6 flex items-center justify-between border-slate-200 shadow-sm bg-white">
            <div>
        ...
        <Card className="lg:col-span-2 border-l-4 border-l-amber-500 shadow-sm relative overflow-hidden bg-white">
          <CardHeader>
            <div className="flex items-center gap-3">
              <div className={hasActions ? "p-2 bg-amber-50 rounded-lg text-amber-600" : "p-2 bg-emerald-50 rounded-lg text-emerald-600"}>
          ...
          <CardContent className="space-y-3 pt-2">
            {actionsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <Button variant="outline" size="sm" onClick={() => setLocation('/admin/properties')}>
              ...
              <div className="flex items-center justify-between">
               ...
               <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{(propStats as any)?.qualityMetrics?.featuredCount || 0} Featured Listings</p>
                  </div>
                <div className="text-right">
                  <p className="font-bold text-slate-900 text-lg">{Math.round((propStats as any)?.qualityMetrics?.averageScore || 0)}</p>
                </div>
              </div>
              
              <div className="pt-4 border-t border-slate-100">
                <Button variant="ghost" className="w-full text-xs text-slate-500 hover:text-slate-900 justify-start px-0" onClick={() => setLocation('/admin/analytics')}>
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Monetization</h3>
        <Card className="p-0 overflow-hidden bg-white border-slate-200 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
             ...
             <div className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => setLocation('/admin/revenue')}>
               <div className="flex items-center gap-3">
                 <Activity className="h-5 w-5 text-slate-400" />
                 <span className="text-slate-700 font-medium group-hover:text-blue-600 transition-colors">Revenue Center</span>
               </div>
