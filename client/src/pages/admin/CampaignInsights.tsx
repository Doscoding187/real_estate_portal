...
import { ArrowLeft, TrendingUp, MousePointer, Users, DollarSign, Eye, BarChart2 } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 capitalize">{campaign.campaignType?.replace('_', ' ')} Campaign</p>
          </div>
        </div>
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{summary.impressions.toLocaleString()}</h3>
              <p className="text-xs text-emerald-600 flex items-center mt-1">
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{summary.clicks.toLocaleString()}</h3>
                ...
                <div className="h-[300px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={dailyData}>
                            ...
                            <Tooltip />
                            <Area yAxisId="left" type="monotone" dataKey="impressions" stroke="#8884d8" fillOpacity={1} fill="url(#colorImpressions)" name="Impressions" />
                            <Area yAxisId="right" type="monotone" dataKey="clicks" stroke="#82ca9d" fillOpacity={1} fill="url(#colorClicks)" name="Clicks" />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </CardContent>
                ...
                <div className="mt-4 space-y-3">
                    {channelPerformance.map((channel, i) => (
                        <div key={i} className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">{((channel.value / summary.impressions) * 100).toFixed(0)}%</span>
                        </div>
                    ))}
                </div>
            </CardContent>
