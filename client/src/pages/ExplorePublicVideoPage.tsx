...
export default function ExplorePublicVideoPage() {
  ...
  const shareVideo = async () => {
    const canonical = data?.canonicalPath || (typeof window !== 'undefined' ? window.location.pathname : '');
    const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}${canonical}` : canonical;
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto w-full max-w-6xl px-4 py-4 sm:px-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-white/70">Related videos</h2>
              ...
              {related.map(item => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="line-clamp-2 text-sm font-medium text-white">{item.title || 'Explore Video'}</p>
                    <p className="mt-1 text-xs text-white/65">{item.actor?.displayName || 'Creator'}</p>
                  </div>
                </button>
              ))}
            </div>
