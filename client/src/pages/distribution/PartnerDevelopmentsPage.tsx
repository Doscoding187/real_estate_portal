                ...
                <p className="text-[11px] text-muted-foreground">
                  ...
                  {formatCurrency(
                    estimateMonthlyInstallment(
                      pickRepresentativePrice(item.priceFrom, item.priceTo),
                    ),
                  )}
                </p>
            ...
            <>
              ...
              <div className="mx-auto w-full max-w-[794px] overflow-hidden bg-white text-slate-950 shadow-xl">
                    ...
                    <tbody>
                      {getBrochureUnitRows(brochureItem).visibleUnits.length ? (
                        getBrochureUnitRows(brochureItem).visibleUnits.map((unit: any) => {
                          const referencePrice = pickRepresentativePrice(
                            unit.priceFrom,
                            unit.priceTo,
                          );
                          const monthlyInstallment = estimateMonthlyInstallment(referencePrice);
                ...
                <section className="border-b border-slate-200 px-10 py-3 text-[10px] leading-4 text-slate-500">
                  Estimates use a {BROCHURE_BOND_YEARS}-year bond at {BROCHURE_ANNUAL_INTEREST_RATE}
                  % annual interest. Qualifying income assumes repayment is{' '}
                  {Math.round(BROCHURE_MIN_INCOME_RATIO * 100)}% of gross monthly income. Figures
                  are indicative and subject to lender approval.
                </section>
