 ...
 *
 * Accessibility fix:
 * - Keep the outer wrapper as a true <article> (role="article")
 * - Use an overlay <button> for click + keyboard interactions
 *   (so ModernCard doesn't switch the article to role="button")
 */
...
export function NeighbourhoodCard({ neighbourhood, onClick, onFollow }: NeighbourhoodCardProps) {
  ...
  const articleLabel = `Neighbourhood: ${neighbourhood.name} in ${neighbourhood.city}`;
  const openLabel = `Open neighbourhood: ${neighbourhood.name} in ${neighbourhood.city}`;

  return (
    <ModernCard
      // ✅ IMPORTANT: do NOT pass onClick to ModernCard
      // Passing onClick makes ModernCard render role="button", breaking role="article" tests
    ...
    >
      {/* ✅ Full-card click/keyboard target (keeps wrapper as article) */}
      <button
        type="button"
        onClick={onClick}
      ...
      {/* All visual content above the overlay button */}
