...
function RefineProvince({ context }: { context: NonNullable<SearchResults['locationContext']> }) {
  ...
  // Filter out the current city if mapped? No, we are in province mode.
  // Sort by Metro status first, then name
      ...
      <div className="flex flex-col gap-1">
        {sortedCities.map(city => (
            <Button
                ...
                onClick={() => setLocation(`/property-for-sale?city=${city.slug || city.name.toLowerCase().replace(/ /g, '-')}`)}
            >
                {city.name}
            </Button>
        ))}
      ...
      <div className="flex flex-col gap-1">
        {limitedSuburbs.map(suburb => (
            <Button
                ...
                variant="ghost"
                className="justify-between h-8 px-2 text-sm font-normal text-slate-600 hover:text-blue-600 hover:bg-blue-50"
                onClick={() => setLocation(`/property-for-sale?suburb=${suburb.slug || suburb.name.toLowerCase().replace(/ /g, '-')}`)}
            >
                {suburb.name}
    ...
    return (
        <div className="mb-6">
             <div className="mb-4 p-3 bg-blue-50 rounded-md border border-blue-100">
                <span className="text-xs text-blue-600 uppercase font-bold tracking-wider">Current Area</span>
