...
export function AgentTopNav() {
  ...
  const workspaceStatusUnavailable = Boolean(onboardingError);
  const workspaceUnlocked =
    !onboardingLoading &&
    !workspaceStatusUnavailable &&
    Boolean(onboardingStatus?.fullFeaturesUnlocked);
  const workspaceSearchLocked =
    !onboardingLoading && !workspaceStatusUnavailable && !workspaceUnlocked;
  const workspaceSearchUnavailable =
    onboardingLoading || workspaceStatusUnavailable || !workspaceUnlocked;
  const canSearch = normalizedSearch.length >= 2 && user?.role === 'agent' && workspaceUnlocked;
  const listingAccessLocked =
    !onboardingLoading &&
    !workspaceStatusUnavailable &&
  ...
  const startListing = () => {
    if (onboardingLoading || workspaceStatusUnavailable || journeyAction.waiting) return;
    if (listingAccessLocked) {
      ...
      <div className="flex min-h-[58px] flex-wrap items-center gap-3 px-4 py-2 md:px-7 xl:h-[58px] xl:flex-nowrap xl:gap-3 xl:py-0">
        ...
        <div className="relative min-w-[220px] flex-1 xl:max-w-[340px]">
          ...
          <Input
            ...
            disabled={workspaceSearchUnavailable}
            placeholder={
              onboardingLoading
                ? 'Preparing workspace search…'
                : workspaceStatusUnavailable
                  ? 'Workspace status unavailable — retry on this page'
                  : workspaceSearchLocked
                    ? journeyAction.waiting
                      ? 'Workspace search unlocks after verification'
                      : `${journeyAction.label} to search your workspace`
                    : 'Search listings, leads, appointments...'
            }
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button
            ...
            disabled={onboardingLoading || workspaceStatusUnavailable || journeyAction.waiting}
          ...
          <NotificationCenter
            enabled={workspaceUnlocked}
            className="h-[34px] w-[34px] rounded-full border border-slate-200 bg-white px-0 text-slate-500 hover:border-[color:color-mix(in_oklab,var(--primary)_24%,white)] hover:bg-white hover:text-[var(--primary)]"
          />
        </div>
      </div>
    </header>
