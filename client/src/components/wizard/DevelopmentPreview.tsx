...
export const DevelopmentPreview: React.FC = () => {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen text-gray-500">Loading preview...</div>;
  if (!data) return <div className="flex items-center justify-center h-screen text-gray-500">No preview data found. Please return to the wizard and click Preview again.</div>;
      ...
      <div className={`relative w-full bg-gray-200 ${isMobile ? 'h-[250px]' : 'h-[500px]'}`}>
        {heroImage?.url ? (
          <img 
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/1200x500?text=Image+Preview';
            }}
          />
        ) : (
        ...
        <div className={`absolute bottom-0 left-0 right-0 ${isMobile ? 'p-4' : 'p-8'} max-w-7xl mx-auto`}>
          <div className="flex items-end justify-between">
            <div className="w-full">
              ...
              <h1 className={`${isMobile ? 'text-2xl' : 'text-4xl md:text-5xl'} font-bold text-white mb-2 leading-tight`}>
                {developmentData?.name || 'Untitled Development'}
              </h1>
              <div className={`flex items-center text-white/90 gap-2 ${isMobile ? 'text-sm' : 'text-lg'}`}>
                ...
                <div key={unit.id} className={`rounded-xl shadow-sm border transition-colors ${isDarkMode ? 'bg-gray-900 border-gray-800 hover:border-blue-800' : 'bg-white border-gray-100 hover:border-blue-200'} ${isMobile ? 'p-4' : 'p-6 flex flex-col md:flex-row gap-6 items-center'}`}>
                  <div className="flex-1">
                    ...
                    <div className={`flex flex-wrap gap-4 text-sm my-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} ${isMobile ? 'gap-y-2' : ''}`}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <Car className="w-4 h-4" /> {unit.parking}
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Maximize className="w-4 h-4" /> {unit.unitSize} m²
                      </div>
                    </div>
