...
import { TrendingUp, Users, Eye, MousePointerClick, Calendar, BarChart3, AlertCircle } from 'lucide-react';
...
const AnalyticsPanel: React.FC = () => {
  ...
  const { data: kpis, isLoading, error, refetch } = trpc.developer.getDashboardKPIs.useQuery({
    timeRange,
    forceRefresh: false
  });
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader>
             <CardTitle className="flex justify-between items-center">
                ...
                <span className="text-sm font-normal text-muted-foreground">Based on {timeRange} activity</span>
             </CardTitle>
          </CardHeader>
          <CardContent>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="space-y-2">
                    ...
                    <div className="flex items-end gap-2">
                        ...
                        <span className={`text-xs mb-1 ${kpis?.trends.conversionRate >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                           {kpis?.trends.conversionRate > 0 ? '+' : ''}{kpis?.trends.conversionRate}%
                        </span>
                    </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-end gap-2">
                        <span className="text-2xl font-bold text-slate-900">{kpis?.affordabilityMatchPercent}%</span>
                         <span className={`text-xs mb-1 ${kpis?.trends.affordabilityMatchPercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                           {kpis?.trends.affordabilityMatchPercent > 0 ? '+' : ''}{kpis?.trends.affordabilityMatchPercent}%
                        </span>
                    </div>
