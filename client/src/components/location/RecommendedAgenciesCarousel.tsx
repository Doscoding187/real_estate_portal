...
export function RecommendedAgenciesCarousel({
  agencies,
  locationName,
}: RecommendedAgenciesCarouselProps) {
  ...
  return (
    <section
      className="recommended-agencies w-full py-8"
      aria-labelledby="recommended-agencies-title"
    >
              ...
              <CarouselItem key={colIndex} className="pl-4 md:basis-1/2 lg:basis-1/3 xl:basis-1/3">
                <div className="flex flex-col gap-4 h-full">
                  {column.map(agency => (
                    <article
                      key={agency.id}
                      className="agency-card bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-500 transition-all duration-300 overflow-hidden flex flex-row p-4 h-[160px] group box-border"
                    >
                      {/* Left Column - Avatar */}
                      ...
                      <div className="agency-info flex flex-col justify-between flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1 mb-1">
                            {agency.badges.map((badge, idx) => (
                              <Badge
                                key={idx}
                                variant="outline"
                                className={`text-[10px] px-1.5 py-0 rounded-full font-bold uppercase tracking-wider ${getBadgeStyle(badge)}`}
                              >
                                {badge}
                              </Badge>
                            ))}
                        ...
                        <div className="flex flex-col gap-1 mt-auto">
                          <div className="flex items-center gap-2 text-xs font-medium text-gray-700">
                            <span className="text-[#DE3831] font-bold">{agency.experience}</span>{' '}
                            Experience
                          </div>
                          <div className="flex items-center gap-2 text-xs font-medium text-gray-700">
