...
export const EntityStatusCard: React.FC<EntityStatusCardProps> = ({
  ...
  if (data.rejectionReasons) {
    try {
      rejectionReasons =
        typeof data.rejectionReasons === 'string'
          ? JSON.parse(data.rejectionReasons)
          : data.rejectionReasons;
    } catch (e) {
      rejectionReasons = [String(data.rejectionReasons)];
    }
  ...
  return (
    <Card
      className={cn(
        'overflow-hidden bg-white border-slate-200 shadow-sm hover:shadow-md transition-all duration-200',
        className,
      )}
    >
      <CardContent className="p-0">
        <div className="flex flex-col sm:flex-row gap-0 sm:gap-6">
          ...
          <div className="w-full sm:w-48 h-48 sm:h-auto bg-slate-100 shrink-0 relative border-r border-slate-100">
            ...
            <div className="absolute top-3 left-3 sm:hidden">{getStatusBadge()}</div>
          </div>
            ...
            <div>
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {quality &&
                    (status === 'draft' || status === 'active' || status === 'published') && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <ul className="text-sm text-red-700 list-disc pl-4 mt-2 mb-2 space-y-1">
                        ...
                        {rejectionReasons.length > 3 && (
                          <li>+ {rejectionReasons.length - 3} more</li>
                        )}
                      </ul>
                      {rejectionNote && (
                        <p className="text-sm text-red-600 mt-2 bg-white/50 p-2 rounded border border-red-100 italic">
                          Note from Admin: "{rejectionNote}"
                        </p>
                      )}
                      <Button
                        size="sm"
