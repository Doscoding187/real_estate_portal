...
export function QualityScoreCard({ qualityScore, qualityBreakdown, tips = [], className }: QualityScoreCardProps) {
  ...
  if (derivedTips.length === 0 && qualityBreakdown) {
      if (qualityBreakdown.imageCount < 10) derivedTips.push(`Add ${10 - qualityBreakdown.imageCount} more photos.`);
      if (qualityBreakdown.descriptionLength < 500) derivedTips.push("Expand description to 500+ characters.");
      if (!qualityBreakdown.hasVideo && !qualityBreakdown.hasVirtualTour) derivedTips.push("Add a video or virtual tour.");
    ...
    <Card className={`overflow-hidden ${className}`}>
      <CardHeader className="pb-3 bg-slate-50 border-b">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className={`${styles.bg} ${styles.text} border ${styles.border}`}>
            {label}
          </Badge>
        </div>
      </CardHeader>
          ...
          <div className="mt-4">
             ...
             >
                <div className="flex items-center gap-2">
                   ...
                   <span className="font-medium text-sm">Optimization Tips ({derivedTips.length})</span>
                </div>
