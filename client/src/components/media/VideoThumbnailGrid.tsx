...
export function VideoThumbnailGrid({ videos, onPlayClick, fallbackImage }: VideoThumbnailGridProps) {
  ...
  const renderTile = (video: VideoMedia, className: string, showPlayIcon = true) => {
    ...
    return (
      <div className={`relative overflow-hidden group/video ${className}`}>
        {thumb ? (
            <img src={thumb} className="w-full h-full object-cover transition-transform duration-700 group-hover/video:scale-105" alt="Video Thumbnail" />
        ) : (
