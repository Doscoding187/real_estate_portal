...
describe('Property Filters Store - URL Synchronization', () => {
  ...
  it('Property 3: should accurately represent filter state in URL parameters', () => {
    fc.assert(
      fc.property(
        ...
        fc.record({
          // Location filters
          province: fc.option(fc.constantFrom('Gauteng', 'Western Cape', 'KwaZulu-Natal', 'Eastern Cape'), { nil: undefined }),
          city: fc.option(fc.constantFrom('Johannesburg', 'Cape Town', 'Durban', 'Pretoria'), { nil: undefined }),
          suburb: fc.option(
            fc.array(fc.constantFrom('Sandton', 'Camps Bay', 'Umhlanga', 'Hatfield'), { minLength: 1, maxLength: 3 }),
            { nil: undefined }
          ),
          
          // Basic filters
          propertyType: fc.option(
            fc.array(fc.constantFrom('house', 'apartment', 'townhouse', 'plot', 'commercial'), { minLength: 1, maxLength: 3 }),
            { nil: undefined }
          ),
          ...
          loadSheddingSolutions: fc.option(
            fc.array(fc.constantFrom('solar', 'generator', 'inverter', 'none'), { minLength: 1, maxLength: 3 }),
            { nil: undefined }
          ),
          
          // Status filters
          status: fc.option(
            fc.array(fc.constantFrom('available', 'under_offer', 'sold', 'let'), { minLength: 1, maxLength: 3 }),
            { nil: undefined }
          ),
        }),
        (filterData) => {
          ...
          if (filterData.loadSheddingSolutions && filterData.loadSheddingSolutions.length > 0) {
            expect(urlParams.get('loadSheddingSolutions')).toBe(filterData.loadSheddingSolutions.join(','));
          }
    ...
    fc.assert(
      fc.property(
        ...
        (originalFilters) => {
          ...
          expect(stateAfterRestore.loadSheddingSolutions).toEqual(stateAfterSet.loadSheddingSolutions);
  ...
  it('should include sort option and view mode in URL parameters', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('price_asc', 'price_desc', 'date_desc', 'date_asc', 'suburb_asc', 'suburb_desc'),
