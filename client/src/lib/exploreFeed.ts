...
function resolveFeedCollection(payload: any): any[] {
  if (Array.isArray(payload)) return payload;
  if (Array.isArray(payload?.items)) return payload.items;
  if (Array.isArray(payload?.shorts)) return payload.shorts;
  if (Array.isArray(payload?.data?.items)) return payload.data.items;
  if (Array.isArray(payload?.data?.shorts)) return payload.data.shorts;
  return [];
}

export function getFeedItems(payload: any): FeedItem[] {
  const items = resolveFeedCollection(payload);
