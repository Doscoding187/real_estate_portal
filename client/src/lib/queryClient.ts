...
export function prefetchExploreFeed(filters: ExploreFeedFilters) {
  ...
  return queryClient.prefetchQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch('/api/explore/getFeed?' + new URLSearchParams({
        ...Object.fromEntries(
...
export function prefetchVideoFeed(filters: ExploreFeedFilters) {
  ...
  return queryClient.prefetchQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/explore/getVideoFeed?' + new URLSearchParams({
        ...Object.fromEntries(
          Object.entries({ ...filters, offset: nextOffset }).filter(([_, v]) => v != null)
        ),
      } as any));
 ...
 */
export function prefetchMapProperties(bounds: {
  north: number;
  south: number;
  east: number;
  west: number;
