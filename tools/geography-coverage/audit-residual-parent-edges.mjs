...
const AUDIT_PATH = 'data/geography-coverage-v0.1/research/residual-parent-edge-audit.v0.1.json';
...
const queue = readJsonl(`${OUTPUT_DIR}/gauteng_review_queue_v0.1.jsonl`).filter(
  entry => entry.reason === 'awaiting_accepted_parent_edge',
);
...
if (audit.queue_reason !== 'awaiting_accepted_parent_edge') {
...
for (const entry of entries) {
  ...
  if (entry.decision !== 'retain_queue') {
    fail(`${entry.factual_location_id} is not retain_queue`);
  }
...
for (const entry of entries) {
  if (!queueIds.has(entry.factual_location_id)) {
    fail(`audit identity ${entry.factual_location_id} is not in the current parent-edge queue`);
  }
}
...
if (failures.length > 0) {
  console.error(`residual-parent-audit: ${failures.length} failure(s)`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(
  `residual-parent-audit: ${queue.length}/${queue.length} queue identities reviewed; 0 proposed parent edges`,
);
