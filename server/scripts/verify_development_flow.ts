  ...
  try {
    ...
    if (!developer) {
      ...
      developer = await db.query.developers.findFirst({
        where: eq(developers.id, devProfileId as number),
      });
    }
    ...
    if (newDev.approvalStatus !== 'draft' && newDev.approvalStatus !== 'planning') {
      // Service defaults to 'planning'
    ...
    const publishedDev = await developmentService.publishDevelopment(
      newDev.id,
      developer.id,
      false,
    ); // false = not trusted (invokes review)
    ...
    const approvedDev = await developmentService.approveDevelopment(newDev.id, admin.id, {
      compliance_checked: true,
    });
    console.log(
      `✅ Approved. Final Status: ${approvedDev.approvalStatus} | Published: ${approvedDev.isPublished}`,
    );
