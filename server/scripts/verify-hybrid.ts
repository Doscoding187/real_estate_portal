...
async function main() {
  ...
  try {
    ...
    data.topLocalities.forEach(loc => {
      ...
      console.log(
        `- ${loc.name}: Price R${loc.avgSalePrice}, Listings: ${loc.propertiesForSale}, Source: ${source}`,
      );
    });
    ...
    if (cityData) {
      ...
      console.log(
        `- Sentinel Source check: ${cityData.stats.totalListings === 0 && cityData.stats.avgSalePrice > 0 ? 'Hybrid (Intel Pricing)' : 'Database'}`,
      );
    } else {
    ...
    const suburbData = await locationPagesService.getSuburbData(
      'gauteng',
      'sandton',
      'morningside',
    );
