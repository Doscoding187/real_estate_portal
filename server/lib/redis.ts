...
export const ExploreCacheKeys = {
  userPreferences: (userId: number) => `${CachePrefix.USER_PREFS}${userId}`,
  ...
  similarProperties: (propertyId: number) => `${CachePrefix.SIMILAR}${propertyId}`,

  boostCampaign: (campaignId: number) => `${CachePrefix.BOOST}campaign:${campaignId}`,

  creatorAnalytics: (creatorId: number, period: string) =>
    `${CachePrefix.ANALYTICS}creator:${creatorId}:${period}`,

  sessionData: (sessionId: string) => `${CachePrefix.SESSION}${sessionId}`,
};
