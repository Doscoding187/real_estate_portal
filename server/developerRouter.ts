...
export const developerRouter = router({
  ...
  createLead: publicProcedure
    .input(
      z.object({
        ...
        unitPriceLabel: z.string().trim().max(80).optional(),
        transactionType: z.enum(['sale', 'rent', 'auction']).or(z.string().trim().max(40)).optional(),
  ...
  completeLeadEvidenceFileUpload: protectedProcedure
    .input(
      z.object({
    ...
    .mutation(async ({ ctx, input }) => {
      const user = requireUser(ctx);
      const profile = await requireDeveloperProfileByUserId(user.id);
      return await completeLeadEvidenceFileUpload({
        developerId: profile.id,
        userId: user.id,
  ...
  getSaleOperatingInventory: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
      }),
    ...
    .query(async ({ ctx, input }) => {
      const profile = await requireDeveloperProfileByUserId(requireUser(ctx).id);
      return await listSaleOperatingInventory({
        developerId: profile.id,
        developmentId: input.developmentId,
      });
    }),

  transitionSaleUnitReservation: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
        unitTypeId: z.string().trim().min(1).max(DEVELOPMENT_UNIT_ID_MAX_LENGTH),
        transition: z.enum(['reserve', 'release']),
    ...
    .mutation(async ({ ctx, input }) => {
      const user = requireUser(ctx);
      const profile = await requireDeveloperProfileByUserId(user.id);
      return await transitionSaleUnitReservation({
        developerId: profile.id,
        developmentId: input.developmentId,
  ...
  markSaleUnitTypeSold: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
        unitTypeId: z.string().trim().min(1).max(DEVELOPMENT_UNIT_ID_MAX_LENGTH),
    ...
    .mutation(async ({ ctx, input }) => {
      const user = requireUser(ctx);
      const profile = await requireDeveloperProfileByUserId(user.id);
      return await markSaleUnitTypeSold({
        developerId: profile.id,
        developmentId: input.developmentId,
        unitTypeId: input.unitTypeId,
        actorUserId: user.id,
  ...
  getRentalOperatingInventory: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
      }),
    ...
    .query(async ({ ctx, input }) => {
      const profile = await requireDeveloperProfileByUserId(requireUser(ctx).id);
      return await listRentalOperatingInventory({
        developerId: profile.id,
        developmentId: input.developmentId,
      });
    }),

  transitionRentalUnitHold: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
        unitTypeId: z.string().trim().min(1).max(DEVELOPMENT_UNIT_ID_MAX_LENGTH),
        transition: z.enum(['hold', 'release']),
        note: z.string().trim().max(1000).optional(),
      }),
    )
    .mutation(async ({ ctx, input }) => {
  ...
  markRentalUnitTypeLet: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
        unitTypeId: z.string().trim().min(1).max(DEVELOPMENT_UNIT_ID_MAX_LENGTH),
    ...
    .mutation(async ({ ctx, input }) => {
      const user = requireUser(ctx);
      const profile = await requireDeveloperProfileByUserId(user.id);
      return await markRentalUnitTypeLet({
        developerId: profile.id,
        developmentId: input.developmentId,
        unitTypeId: input.unitTypeId,
        actorUserId: user.id,
  ...
  activateAuctionLot: protectedProcedure
    .input(
      z.object({
        developmentId: z.number().int().positive(),
        unitTypeId: z.string().trim().min(1).max(DEVELOPMENT_UNIT_ID_MAX_LENGTH),
    ...
    .mutation(async ({ ctx, input }) => {
      const user = requireUser(ctx);
      const profile = await requireDeveloperProfileByUserId(user.id);
      return await activateAuctionLot({
        developerId: profile.id,
        developmentId: input.developmentId,
        unitTypeId: input.unitTypeId,
        actorUserId: user.id,
