...
guardedDescribe('agency listing performance MVP persisted integration', () => {
  it('keeps canonical performance, review, revision, access, and publication contracts intact', async () => {
    ...
    await db.update(listingAnalytics).set({ totalViews: 37, uniqueVisitors: 25, totalLeads: 1 } as any).where(eq(listingAnalytics.listingId, canonical.id));
    const canonicalAnalytics = await db.select({ id: listingAnalytics.id, listingId: listingAnalytics.listingId, totalViews: listingAnalytics.totalViews, uniqueVisitors: listingAnalytics.uniqueVisitors, totalLeads: listingAnalytics.totalLeads }).from(listingAnalytics).where(eq(listingAnalytics.listingId, canonical.id));
    expect(canonicalAnalytics).toHaveLength(1);
    expect(canonicalAnalytics[0]).toMatchObject({ listingId: canonical.id, totalViews: 37, uniqueVisitors: 25, totalLeads: 1 });
    ...
    const legacyOrdering = await publishedListing(managerId, agencyId, assignedAgentId, `${suffix}-legacy-ordering`);
    const tiedLastUpdated = '2026-07-14 12:00:00';
    await db.update(listingAnalytics).set({ totalViews: 11, uniqueVisitors: 10, totalLeads: 0, lastUpdated: tiedLastUpdated } as any).where(eq(listingAnalytics.listingId, legacyOrdering.id));
    await db.insert(listingAnalytics).values({ listingId: legacyOrdering.id, totalViews: 73, uniqueVisitors: 60, totalLeads: 2, lastUpdated: tiedLastUpdated } as any);
    const legacyOrderingSnapshot = await manager.agency.getListingPerformance({ listingId: legacyOrdering.id });
