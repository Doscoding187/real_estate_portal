...
async function getPlatformSettingsColumnMode(db: any): Promise<PlatformSettingsColumnMode> {
  ...
  try {
    await db.execute(sql.raw('SELECT `setting_key`, `setting_value` FROM platform_settings LIMIT 1'));
  ...
  throw new Error('Unsupported platform_settings schema. Expected key/value or setting_key/setting_value.');
}
