...
import { db } from '../db';
import { developmentPartners, developmentLeadRoutes, developments, developerBrandProfiles } from '../../drizzle/schema';
...
export async function removePartner(
  ...
  await db.update(developmentLeadRoutes)
    .set({ isActive: 0 })
    .where(and(
  ...
  await db.delete(developmentPartners)
    .where(and(
...
export async function getPartners(developmentId: number) {
  ...
  .leftJoin(developerBrandProfiles, eq(developmentPartners.brandProfileId, developerBrandProfiles.id))
...
async function createAgencyLeadRoute(
  ...
  const [dev] = await db.select({ ownerBrandProfileId: developments.developerBrandProfileId })
    .from(developments)
