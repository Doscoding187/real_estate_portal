...
export const locationAnalyticsService = {
  ...
  async calculatePriceStats(locationId: number): Promise<PriceStats> {
    ...
    const avgSalePrice =
      saleListings.length > 0
        ? saleListings.reduce((sum: number, l) => sum + Number(l.askingPrice), 0) /
          saleListings.length
        : null;
    ...
    const avgRentalPrice =
      rentalListings.length > 0
        ? rentalListings.reduce((sum: number, l) => sum + Number(l.monthlyRent), 0) /
  ...
  async calculateMarketActivity(locationId: number): Promise<MarketActivity> {
    ...
    const avgDaysOnMarket =
      daysOnMarket.length > 0
        ? Math.round(
            daysOnMarket.reduce((sum: number, days) => sum + days, 0) / daysOnMarket.length,
          )
        : null;
  ...
  async getLocationStatistics(locationId: number): Promise<LocationStatistics> {
    ...
    const developmentCount = await db
      .select({ count: sql<number>`count(*)` })
      .from(developments)
      .where(and(locationFilter, eq(developments.isPublished, 1)));
   ...
   */
  async getTrendingSuburbs(limit: number = 10): Promise<
    Array<{
      id: number;
      name: string;
      slug: string;
  ...
  > {
    ...
    try {
      ...
      const candidateLocations = await db
        ...
        .from(locations)
        .where(and(eq(locations.type, targetLocation.type), sql`${locations.id} != ${locationId}`))
      ...
      for (const candidate of candidateLocations) {
        try {
          ...
          console.error(
            `[LocationAnalytics] Error calculating similarity for location ${candidate.id}:`,
            error,
          );
