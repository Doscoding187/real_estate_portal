...
function statusOrMissing(value: unknown): DealRequirementStatus {
  ...
  if (text === 'missing' || text === 'uploaded' || text === 'pending_review' || text === 'verified' || text === 'rejected' || text === 'waived') {
    return text;
  }
  return 'missing';
}
...
export function assertValidStatusTransition(
  ...
  if (toStatus === 'uploaded') {
    if (fromStatus === 'verified') {
      throw new TRPCError({ code: 'BAD_REQUEST', message: 'Cannot upload after verified without reset or waiver.' });
    }
    return;
  }
  ...
  if (toStatus === 'verified') {
    if (fromStatus !== 'pending_review') {
      throw new TRPCError({ code: 'BAD_REQUEST', message: 'Cannot verify unless requirement is pending_review.' });
    }
  ...
  if (toStatus === 'rejected') {
    if (fromStatus !== 'uploaded' && fromStatus !== 'pending_review') {
      throw new TRPCError({ code: 'BAD_REQUEST', message: 'Cannot reject unless requirement is uploaded or pending_review.' });
    }
    if (actorRole !== 'manager' && actorRole !== 'developer_admin') {
  ...
  if (toStatus === 'missing') {
    if (actorRole !== 'manager' && actorRole !== 'developer_admin' && actorRole !== 'system') {
      throw new TRPCError({ code: 'FORBIDDEN', message: 'Only reviewers can reset to missing.' });
    }
  }
}

export async function transitionDealRequirementStatus(input: DealRequirementTransitionInput) {
  ...
  return db.transaction(async tx => {
    ...
    if (!deal) throw new TRPCError({ code: 'NOT_FOUND', message: 'Distribution deal not found.' });
    ...
    if (!requirement || Number(requirement.developmentId) !== Number(deal.developmentId)) {
      throw new TRPCError({ code: 'BAD_REQUEST', message: 'Requirement is not valid for this deal development.' });
    }

    const [existing] = await tx
      .select({ id: dealRequirementStatuses.id, status: dealRequirementStatuses.status })
    ...
    const isReviewStatus =
      input.toStatus === 'verified' || input.toStatus === 'rejected' || input.toStatus === 'waived';
    ...
    if (!existing) {
      ...
      await tx.update(dealRequirementStatuses).set(updatePayload).where(eq(dealRequirementStatuses.id, Number(existing.id)));
    }
...
export async function autoVerifyRequirementFromLinkedDevelopmentDocument(input: {
  ...
  const [requirement] = await db
    .select({ linkedDevelopmentDocumentId: applicationRequirements.linkedDevelopmentDocumentId })
    .from(applicationRequirements)
    .where(eq(applicationRequirements.id, input.requirementId))
    .limit(1);
  ...
  if (!document || Number(document.isActive) !== 1) return null;
