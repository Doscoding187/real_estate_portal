...
export type StartLeadFunnelSessionInput = Partial<Omit<Attribution, 'sourceType'>> & {
...
export async function startLeadFunnelSession(
  ...
  const ttlHours = Math.max(1, Math.min(Number(input.ttlHours || DEFAULT_SESSION_TTL_HOURS), 24 * 90));
