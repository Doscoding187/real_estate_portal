...
describe('Development Service - Property Tests', () => {
  ...
  async function cleanupTestData(developerId: number) {
    ...
    const devs = await db.select().from(developments).where(eq(developments.developerId, developerId));
  ...
  it.prop([
    ...
    async (userId, amenities) => {
      ...
      try {
        ...
        const retrievedAmenities = typeof retrieved?.amenities === 'string' 
  ...
  it.prop([
    ...
    fc.record({
      ...
      developmentType: fc.constantFrom('residential', 'commercial', 'mixed_use', 'estate', 'complex'),
      city: fc.string({ minLength: 2, maxLength: 50 }),
      province: fc.string({ minLength: 2, maxLength: 50 }),
      description: fc.option(fc.string({ minLength: 10, maxLength: 500 })),
    }),
  ])(
    'Development profile captures all required fields',
    async (userId, developmentData) => {
      let developerId: number | null = null;
