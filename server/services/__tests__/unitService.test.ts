...
describe('Unit Service - Property Tests', () => {
  ...
  it.prop([
    ...
    fc.constantFrom('available', 'reserved', 'sold'),
  ])(
    'Property 5: Unit status transitions are valid',
    async (userId, newStatus) => {
      ...
      try {
        ...
        // Update unit status
        const updatedUnit = await unitService.updateUnitStatus(
          unit.id,
          developer.id,
          newStatus
        );
  ...
  it.prop([
    ...
    'Availability summary accurately reflects unit statuses',
    async (userId, unitCount) => {
      let developerId: number | null = null;
