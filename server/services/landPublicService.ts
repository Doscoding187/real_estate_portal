...
export function publicParcelComposition(parcelCount: number) {
  return parcelCount === 1
    ? 'This site comprises 1 parcel.'
    : `This site comprises ${parcelCount} parcels.`;
}

async function resolveGeography(
  input: LandPublicSearchInput,
): Promise<PublicSearchQueryBoundary | undefined> {
  if (input.searchAreaId) {
    ...
    if (
      ...
      !resolution.definition.supportedJourneys.includes('plot_land')
    ) {
  ...
  const ids = Array.from(
    new Set([...(input.locationIds || []), ...(input.locationId ? [input.locationId] : [])]),
  );
  ...
  const resolved = await Promise.all(
    ids.map(id => locationResolver.resolvePublicLocation({ locationId: id })),
  );
  if (resolved.some(item => item.status !== 'resolved' || !item.location))
  ...
  if (!boundary)
    throw new Error('Land multi-location search requires exact sibling canonical locations.');
  return boundary;
}
...
function geographyPredicate(boundary: PublicSearchQueryBoundary | undefined) {
  ...
  const members =
    boundary.kind === 'canonical_locations'
      ? boundary.members
      : getSearchAreaQueryMembers(boundary);
...
async function passport(db: Awaited<ReturnType<typeof database>>, assetId: number) {
  ...
  const assertions = claims.length
    ? await db
        ...
        .where(eq(landClaims.landAssetId, assetId))
    : [];
...
export async function searchPublicLand(input: LandPublicSearchInput = {}) {
  ...
  if (geography)
    conditions.push(
      sql`EXISTS (SELECT 1 FROM land_asset_parcels gap INNER JOIN land_parcels gp ON gap.parcel_id = gp.id WHERE gap.land_asset_id = ${landAssets.id} AND ${geography})`,
    );
  if (input.minPrice !== undefined)
    conditions.push(gte(listings.askingPrice, String(input.minPrice)));
  if (input.maxPrice !== undefined)
    conditions.push(lte(listings.askingPrice, String(input.maxPrice)));
  const parcelSummary = db
    .select({
  ...
  const rows = await db
    .select({
      ...
      extentM2: parcelSummary.extentM2,
    ...
    .innerJoin(parcelSummary, eq(parcelSummary.landAssetId, landAssets.id))
  ...
  return publicRows
    ...
    .filter(
      row =>
        (!input.minSize || Number(row.extentM2) >= input.minSize) &&
        (!input.maxSize || Number(row.extentM2) <= input.maxSize),
    );
}
...
export async function resolvePublicLandLeadCustody(listingId: number) {
  ...
  return isPublicLandEligible({
    ...
    ? { listingId: row.listingId, agentId: row.agentId, agencyId: row.agencyId }
    : null;
}
export async function publicLandDetail(slug: string) {
  const results = await searchPublicLand();
