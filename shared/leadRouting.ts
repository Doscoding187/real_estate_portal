...
const optionalNonNegativeInt = z.number().int().min(0).max(100_000_000).nullable().optional();

export const QualificationProfileAnswersSchema = z.object({
  grossMonthlyIncome: optionalNonNegativeInt,
  grossMonthlyIncomeRange: optionalTrimmedString(80),
  coApplicantIncome: optionalNonNegativeInt,
  employmentType: EmploymentTypeSchema.nullable().optional(),
  buyingMode: BuyingModeSchema.default('unsure'),
  preferredProvince: optionalTrimmedString(100),
  preferredCity: optionalTrimmedString(100),
  preferredSuburb: optionalTrimmedString(100),
  targetPriceMin: optionalNonNegativeInt,
  targetPriceMax: optionalNonNegativeInt,
  creditReportStatus: CreditReportStatusSchema.nullable().optional(),
  buyingTimeline: optionalTrimmedString(120),
  estimatedBondAmount: optionalNonNegativeInt,
  metadata: z.record(z.string(), z.unknown()).nullable().optional(),
});

export type QualificationProfileAnswers = z.infer<typeof QualificationProfileAnswersSchema>;

export const SaveQualificationProfileInputSchema = QualificationProfileAnswersSchema.extend({
  sessionToken: z.string().trim().min(20).max(96).nullable().optional(),
  sessionId: z.number().int().positive().nullable().optional(),
  buyerLeadId: z.number().int().positive().nullable().optional(),
}).refine(input => Boolean(input.sessionToken || input.sessionId || input.buyerLeadId), {
  message: 'A session token, session id, or buyer lead id is required.',
  path: ['sessionToken'],
});

export type SaveQualificationProfileInput = z.infer<typeof SaveQualificationProfileInputSchema>;

export const CaptureBuyerLeadInputSchema = z
  .object({
    sessionToken: z.string().trim().min(20).max(96).nullable().optional(),
    sessionId: z.number().int().positive().nullable().optional(),
    ...
    privacyPolicyVersion: z.string().trim().max(40).nullable().optional(),
    notes: z.string().trim().max(2000).nullable().optional(),
    platformLeadId: z.number().int().positive().nullable().optional(),
    metadata: z.record(z.string(), z.unknown()).nullable().optional(),
  })
...
export type CaptureBuyerLeadInput = z.infer<typeof CaptureBuyerLeadInputSchema>;

export const RecordLeadDevelopmentMatchSchema = z.object({
  developmentId: z.number().int().positive(),
  matchScore: z.number().min(0).max(100),
  matchLabel: DevelopmentMatchLabelSchema.default('needs_review'),
  matchReasons: z.array(z.record(z.string(), z.unknown())).nullable().optional(),
  incomeEligible: z.boolean().default(false),
  locationMatch: z.boolean().default(false),
  campaignEligible: z.boolean().default(false),
  distributionReady: z.boolean().default(false),
  submissionAllowed: z.boolean().default(false),
});

export const RecordLeadDevelopmentMatchesInputSchema = z.object({
  buyerLeadId: z.number().int().positive(),
  sessionId: z.number().int().positive().nullable().optional(),
  campaignId: z.number().int().positive().nullable().optional(),
  sourceType: LeadSourceTypeSchema.nullable().optional(),
  selectedDevelopmentId: z.number().int().positive().nullable().optional(),
  matches: z.array(RecordLeadDevelopmentMatchSchema).min(1).max(12),
});

export type RecordLeadDevelopmentMatchesInput = z.infer<
  typeof RecordLeadDevelopmentMatchesInputSchema
>;

export const LeadImportRowSchema = z.object({
  externalLeadId: optionalTrimmedString(120),
  ...
  preferredArea: optionalTrimmedString(160),
  grossMonthlyIncomeRange: optionalTrimmedString(80),
  buyingMode: optionalTrimmedString(80),
  employmentType: optionalTrimmedString(120),
  creditReportStatus: optionalTrimmedString(120),
  buyingTimeline: optionalTrimmedString(120),
  preferredContactMethod: optionalTrimmedString(80),
  contactPermission: z.union([z.boolean(), z.string(), z.number()]).nullable().optional(),
  marketingConsent: z.union([z.boolean(), z.string(), z.number()]).nullable().optional(),
...
export const LeadImportBatchInputSchema = z.object({
  sourceType: LeadSourceTypeSchema.default('manual'),
  campaignId: z.number().int().positive().nullable().optional(),
  attribution: AttributionSchema.partial().nullable().optional(),
  privacyPolicyVersion: z.string().trim().max(40).nullable().optional(),
  defaultContactPermission: z.boolean().default(false),
  defaultMarketingConsent: z.boolean().default(false),
  rows: z.array(LeadImportRowSchema).min(1).max(200),
});

export type LeadImportBatchInput = z.infer<typeof LeadImportBatchInputSchema>;
