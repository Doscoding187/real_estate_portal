...
export function assertCommercialEconomicsInput(item: CommercialEconomicsInput): void {
  ...
  if (computable && (item.amountMinor === null || item.chargeBasis === null)) {
    throw new Error(`${item.componentCode}: ${item.valueState} economics require an amount and charge basis.`);
  }
  if (
    !computable &&
    (item.amountMinor !== null || item.rangeMaximumMinor !== null || item.chargeBasis !== null)
  ) {
    throw new Error(`${item.componentCode}: ${item.valueState} economics cannot carry a cost value.`);
  }
  if (
    item.rangeMaximumMinor !== null &&
    (item.amountMinor === null || item.rangeMaximumMinor < item.amountMinor)
  ) {
    throw new Error(`${item.componentCode}: economics range maximum must be greater than or equal to its amount.`);
  }
}
...
export function assertCommercialAvailabilityFreshness(input: {
  ...
  if (
    ...
    throw new Error('Confirmed availability requires confirmation source, timestamp and reconfirmation deadline.');
  }
  ...
  if (
    ...
    (!input.reconfirmationDueAt || new Date(input.reconfirmationDueAt) < new Date(input.lastConfirmedAt))
  ) {
    throw new Error('Availability reconfirmation deadline must be on or after the confirmation timestamp.');
  }
}
...
export function deriveCommercialMonthlyOccupancyCost(input: {
...
}): { components: CommercialCalculatedCost[]; monthlyMinimumMinor: number; monthlyMaximumMinor: number; unknownComponentCodes: string[] } {
  ...
  for (const item of input.economics) {
    ...
    if (item.valueState === 'not_applicable' || item.amountMinor === null || !item.chargeBasis) continue;
